      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500"
          />
        </div>
        ...
        <div className="space-y-3">
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-slate-400">暂无匹配的真题资源</div>
          ) : (
            filtered.map((item) => (
              <div
                key={item.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {item.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-1.5 py-0.5 rounded text-[11px] bg-slate-50 text-slate-500"
                      >
                        {tag}
                      </span>
                    ))}
